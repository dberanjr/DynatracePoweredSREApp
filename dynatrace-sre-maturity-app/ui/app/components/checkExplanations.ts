...
export const CHECK_EXPLANATIONS: Record<string, string> = {
  ...
  "5. Kubernetes":
    "Counts Kubernetes clusters (dt.entity.kubernetes_cluster) whose name's leading token (before the first '-') matches this AppCI, since clusters follow a '<appci>-<region>-<env>' naming convention. N/A when no clusters match (not every app runs on K8s).",
  "6. Cloud":
    "Counts AWS, Azure, and GCP cloud resources tagged with this applicationci, sourced from smartscapeNodes (the same unified inventory the Clouds app uses) across all resource types. N/A when no cloud resources are tagged.",
  "7. RUM / Synthetics":
  ...
  "6. Critical Services Tagged":
    "Whether this app's critical services are tagged in Dynatrace, so ITSM can route them at priority. STUB: the check is hardcoded to n/a and excluded from the L2 denominator (score is out of 5, not 6). The pipeline it was waiting on now exists - /lookups/critical_services (underscore) carries 9,072 service rows across 361 AppCIs with severity and business impact - but the check has not been wired to it yet, and only ~38% of rows resolve to real entity IDs.",
  ...
  "2. Dynamic Scaling / K8s Autoscaling":
    "Counts real autoscaling constructs tagged with this ApplicationCI, from the same smartscapeNodes inventory the Clouds app uses: EC2/EKS Auto Scaling Groups, Application Auto Scaling scalable targets (ECS service autoscaling), and EKS managed nodegroups. Pass = at least one autoscaling target. Fail = the app has cloud resources but none of them autoscale. N/A = the app has no cloud footprint at all. KNOWN GAP: in-cluster autoscalers (HPA, KEDA, Karpenter) are not counted - they live in Kubernetes workload YAML and each needs its own evaluation.",
